;;; tng-r4.el --- ThiNG r4 code editing commands for Emacs

;; Add code like the following to your .emacs to install:
;; (autoload 'tng-r4-mode "...path.to.wherever.you.put.this.file.../tng-r4.el" nil t)
;; (setq auto-mode-alist (cons '("\\.tng\\'" . tng-r4-mode)
;;                             auto-mode-alist))
...
(defvar tng-r4-mode-syntax-table nil
  "Syntax table in use in tng-r4-mode buffers.")

(if tng-r4-mode-syntax-table
    ()
  (setq tng-r4-mode-syntax-table (make-syntax-table))
  (cond ((memq '8-bit c-emacs-features) ;; XEmacs
	 (modify-syntax-entry ?\" "! " tng-r4-mode-syntax-table))
	((memq '1-bit c-emacs-features) ;; Proper Emacs
	 (modify-syntax-entry ?\" "! " tng-r4-mode-syntax-table)))
  (modify-syntax-entry ?_ "_" tng-r4-mode-syntax-table)
  (mapcar #'(lambda (x) (modify-syntax-entry x "." tng-r4-mode-syntax-table))
	  '(?, ?\; ?- ?+ ?= ?| ?/ ?? ?. ?< ?> ?* ?& ?^ ?% ?$ ?# ?@ ?! ?` ?~))
  (modify-syntax-entry ?\' "\"" tng-r4-mode-syntax-table))

(defconst tng-r4-font-lock-keywords
  (eval-when-compile
    (list
 ...
 "Default expressions to highlight in TNG-R4 mode.")
...
(defun tng-mode-variables ()
  ...
  (setq font-lock-defaults '(tng-r4-font-lock-keywords nil nil ((?_ . "w")))))

;;;###autoload
(define-derived-mode tng-r4-mode c-mode "TNG-R4"
  "Major mode for editing TNG-R4 code.
This is much like C mode except for the syntax of comments.  Its keymap
inherits from C mode's and it has the same variables for customizing
indentation.  It has its own abbrev table and its own syntax table.

Turning on TNG-R4 mode runs `tng-r4-mode-hook'." ;; actually a lie
  (tng-mode-variables))
